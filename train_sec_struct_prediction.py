...
from data.downstream.secondary_structure.datamodule import SecondaryStructureDataModule, SUPPORTED_DATASETS
from utils.sec_struct import prob_mat_to_sec_struct, ss_precision, ss_recall, ss_f1, save_to_ct
from utils.finetune_callback import GradualUnfreezing
from model.downstream import SecStructPredictionHead
from model.model import RIBOX
...
class SecStructPredictionWrapper(pl.LightningModule):
    def __init__(
        ...
        self.cfg, self.default_config = model_cfg(lm_config)
        self.lm = RIBOX(self.cfg)
        self.pred_head = SecStructPredictionHead(self.default_config['model']['embedding'].embedding_dim, num_blocks=num_resnet_blocks)
    ...
    def forward(self, tokens):
        x = self.lm.headless_forward(tokens)
...
def main(args):
    ...
    if args.pretrained_ribox_weights:
        model.load_pretrained_lm_weights(args.pretrained_ribox_weights)

    if args.init_params:
        model.load_state_dict(torch.load(args.init_params))

    # Data
    alphabet = Alphabet(**model.default_config['alphabet'])
...
if __name__ == "__main__":
    ...
    # Model
    parser.add_argument(
        "--lm_config", type=str, default="M",
        help="Language model configuration"
    )
    parser.add_argument(
        "--pretrained_ribox_weights", type=str, default=None,
        help="Path to the pretrained RIBOX model weights"
    )
    parser.add_argument(
