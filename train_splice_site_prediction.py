...
from data.downstream.splice_site_prediction.datamodule import SpliceSiteDataModule
from model.model import RIBOX
from model.downstream import SpliceSitePredictionHead
from config import model_cfg
import utils.splice_site_metrics as ss_pred_metrics
...
class SpliceSitePredictionWrapper(pl.LightningModule):
    def __init__(
        ...
        self.ribox = RIBOX(self.cfg)
        self.pred_head = SpliceSitePredictionHead(
            c_in=self.default_config['model']['embedding'].embedding_dim,
    ...
    def load_pretrained_ribox_weights(self, pretrained_weights_path):
        self.ribox.load_state_dict(torch.load(pretrained_weights_path))

    def forward(self, tokens):

        x = self.ribox.headless_forward(tokens)
        #x[:, 0]
        mask = (tokens != 1)
        mask = mask.unsqueeze(-1)
        x = x * mask
        out = torch.sum(x, dim=1)/torch.sum(mask, dim=1)
...
def main(args):
    ...
    if args.pretrained_ribox_weights:
        model.load_pretrained_ribox_weights(args.pretrained_ribox_weights)
