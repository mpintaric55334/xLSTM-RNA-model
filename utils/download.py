...
def _get_download(url: str, local_dir_path: Path) -> Path:
    ...
    total_size = int(r.headers.get('Content-Length', 0)) / MEGABYTE

    # Handle the case where Content-Length is not provided
    if total_size == 0:
        total_size = None  # Disable the total size in tqdm

    os.makedirs(local_dir_path, exist_ok=True)
    with open(local_file_path , 'wb') as f:
        with tqdm(total=total_size, unit="MB") as progress_bar:
